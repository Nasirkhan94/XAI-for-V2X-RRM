...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_neighbor):
        ...
        self.V2V_power_dB_List = [23 , 10, 5, -100]  # the power levels
    ...
    def Compute_Performance_Reward_Train(self, actions_power):
        ...
        qos= np.array([4e-5 ,3e-4 , 2e-3, 1e-4,4e-5 ,3e-4 , 2e-3, 1e-4]).reshape(V2V_Rate.shape)
    ...
    def Compute_Performance_Reward_Test_rand(self, actions_power):
        ...
        reward_elements[ reliability.reshape(effec_rate.shape) > 0] = 1  ### a reward element of 1 if payload is zero/negative means all bits delivered equivalent to beta
    ...
    def act_for_training(self, actions):
        ...
        # qos= np.array([4e-5 ,3e-4 , 2e-3, 1e-4]).reshape(V2V_Rate.shape)
        
        # reward = lambdda * np.sum(V2I_Rate) / (self.n_Veh * 10) + (1 - lambdda) *np.sum(effec_rate* (1-qos))/ (self.n_Veh*5) - np.sum(np.logical_not(reward_elements)) / (self.n_Veh*5 )
    ...
    def act_for_testing(self, actions):       
        ...
        return V2I_Rate, V2V_success, V2V_Rate,  effec_rate, reward
