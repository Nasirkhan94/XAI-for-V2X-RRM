...
def get_state(env, idx=(0,0) ):
    ...
    # Extract the last 19 elements of feature_ranking as indices.. 
...
def get_state_test(env, idx=(0,0), ind_episode=1, epsi=0.02):
    ...
    V2I_fast_names =[ 'V2I_(strongest)', 'V2I_(2nd strongest)', 'V2I_(3rd strongest)', 'V2I_(weakest)']
    V2I_fast_dict= env.create_state_space_dictionary( V2I_fast_names, np.sort(   V2I_fast.reshape(-1))[::-1] )
    sorted_V2I= dict(sorted(  V2I_fast_dict.items(), key=lambda item: item[1], reverse=True))
    # print(sorted_V2I)
    ...
    V2V_fast = (env.V2V_channels_with_fastfading[:, env.vehicles[idx[0]].destinations[idx[1]], :] )
    V2V_fast_names =['V2V_(strongest)', 'V2V_(2nd strongest)', 'V2V_(3rd strongest)','V2V_(4th strongest)',\
                      'V2V_(5th strongest)','V2V_(6th strongest)',\
                          'V2V_(7th strongest)','V2V_(8th strongest)','V2V_(9th strongest)',\
                              'V2V_(10th strongest)','V2V_(11th strongest)' ,'V2V_(12th strongest)','V2V_(13th strongest)',\
                                'V2V_(14th strongest)','V2V_(15th strongest)', 'V2V_(weakest)']
    V2V_fast_dict= env.create_state_space_dictionary( V2V_fast_names, np.sort(   V2V_fast.reshape(-1))[::-1] )
    sorted_V2V= dict(sorted(  V2V_fast_dict.items(), key=lambda item: item[1], reverse=True))
    # print(sorted_V2V)
    ...
    V2V_interference = (-env.V2V_Interference_all[idx[0], idx[1], :] )
    V2V_intrfr_names =['Interference_(strongest)', 'Interference_(2nd strongest)', 'Interference_(3rd strongest)', 'Interference_(weakest)']
    V2V_intrfr_dict= env.create_state_space_dictionary( V2V_intrfr_names, np.sort(    V2V_interference.reshape(-1))[::-1] )
    sorted_V2V_intrfr= dict(sorted(  V2V_intrfr_dict.items(), key=lambda item: item[1], reverse=True))
...
if IS_TEST:
   ...
   # Specify the folder where the files will be saved
    ...
    feature_names_list=['V2N_(strongest)', 'V2N_(2nd strongest)', 'V2N_(3rd strongest)', 'V2N_(weakest)', 'V2V_(strongest)', 'V2V_(2nd strongest)', 'V2V_(3rd strongest)', 'V2V_(4th strongest)', 'V2V_(5th strongest)', 'V2V_(6th strongest)', 'V2V_(7th strongest)', 'V2V_(8th strongest)', 'V2V_(9th strongest)', 'V2V_(10th strongest)', 'V2V_(11th strongest)', 'V2V_(12th strongest)', 'V2V_(13th strongest)', 'V2V_(14th strongest)', 'V2V_(15th strongest)', 'V2V_(weakest)', 'Interference_(strongest)', 'Interference_(2nd strongest)', 'Interference_(3rd strongest)', 'Interference_(weakest)']
    ...
    for i in range(n_veh):     
        for j in range(n_neighbor):
            ...
            background =  SHAP_x[np.random.choice( SHAP_x.shape[0], 5000, replace=False)]
    ...
    plt.figure(figsize=(10, 8))  # Adjust the width and height as needed
    shap.summary_plot(SHAP_avg, plot_type='bar', max_display=1000, feature_names=feature_names_list, color='#0087CE')
    # shap.summary_plot(np.apply_along_axis(softmax, axis=1, arr=SHAP_avg), plot_type='bar', max_display=1000, feature_names=feature_names_list, color='#4DA414')
